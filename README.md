...
### CRUD
- 소프트웨어가 가지는 기본적인 데이터 처리 기능인 Create(생성), Read(읽기), Update(갱신), Delete(삭제)를 묶어서 일컫는 말
- 대부분의 웹 서비스의 기반이 되는 개념

### CRUD의 흐름
클라이언트(입력/요청) -> 서버(검증/연산/요청) -> 데이터베이스(생성,조회,수정,삭제 *CRUD) -> 서버(검증/연산/요청) -> 클라이언트(입력/요청)

요청을 위한 폼(html)요청 -> 폼(html)응답 -> 데이터 요청(데이터베이스) -> 데이터 응답(데이터베이스)
...
- 인증/권한 : 로그인이 필요하거나, 관리자만 생성할 수 있는 등의 경우 확인 ex)글쓰기, 공지사항 작성
- 데이터 유효성: 클라이언트가 입력한 데이터가 유효한지 검증 ex)나이, 전화번호, 파일 
